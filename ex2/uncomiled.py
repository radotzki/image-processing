...
def imGradSobel(img):
    (r, c) = img.shape
    imgPad = np.zeros((r + 2, c + 2), np.float32, **None)
...
def applyAffineTransToImage(src, affineT):
    ...
    coords = np.column_stack((X.flatten(), Y.flatten(), np.ones(X.flatten().shape))).T
...
def deformationBetweenLines(img, Q1, P1, Q2, P2, a, b = (1, 1)):
    epsilon = 0.01
    r = img.shape[0]
    c = img.shape[1]
    ...
    alpha = np.dot(R2 - np.matlib.repmat(P2, R2.shape[0], 1), u2) / np.linalg.norm(Q2 - P2)
    beta = np.dot(R2 - np.matlib.repmat(P2, R2.shape[0], 1), v2)
    normP1Q1 = np.linalg.norm(Q1 - P1)
    R1 = P1 + np.multiply(alpha * normP1Q1[(:, np.newaxis)], np.matlib.repmat(u1, alpha.shape[0], 1)) + np.multiply(beta[(:, np.newaxis)], np.matlib.repmat(v1, beta.shape[0], 1))
